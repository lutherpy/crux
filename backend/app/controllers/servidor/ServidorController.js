...
async function getServidores(req, res) {
  try {
    ...
    const result = await client.query(
      "SELECT s.id AS servidor_id, s.name AS servidor_name, s.ip_address AS servidor_ip, s.sistema_operacional AS servidor_sop, a.id AS aplicacao_id, a.name AS aplicacao_name, a.descricao AS aplicacao_descricao, d.name AS departamento_name FROM servidor s LEFT JOIN aplicacao_servidor aps ON s.id = aps.servidor_id LEFT JOIN aplicacao a ON aps.aplicacao_id = a.id LEFT JOIN departamento d ON a.departamento_id = d.id ORDER BY s.id, a.id DESC;"
    );
    client.release();
